...
# Suprimindo os avisos específicos de FutureWarning
warnings.filterwarnings("ignore", message="FutureWarning: Downcasting object dtype arrays on .fillna, .ffill, .bfill is deprecated")
...
def merge_operacoes(df, other_df):
    ...
    if not isinstance(df, pd.DataFrame) or not isinstance(other_df, pd.DataFrame):
        raise ValueError("Os argumentos df e other_df devem ser do tipo DataFrame do pandas")

    if df.empty and other_df.empty:
        return pd.DataFrame(columns=colunas_obrigatorias())

    ultima_data = datetime.date.min

    if not df.empty:
        ultima_data = df[df['aquisicao_via'].str.upper() == 'HOMEBROKER']['data'].max()

    if not other_df.empty:
        if not isinstance(df, pd.DataFrame):
            df = pd.DataFrame(df)  # Convertendo para DataFrame do pandas se não for

        other_df_filtered = other_df.loc[other_df['data'] > ultima_data, :]
        
        # Suprimindo o aviso
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            df = pd.concat([df, other_df_filtered], ignore_index=True)
